...
typedef struct{
	...
	double eval;
	int win;
	int nitch; /*-1...所属ニッチなし 1~...所属しているニッチ番号*/
	int obj[No];
	double dis[No];
	int Neigh_List1[Kp];
	int Neigh_List2[Ko];
...
main(){
	...
	while(end_flag){
		...
		while(while_flag){
			...
			for(i=0;pop[MainPare_n].Neigh_List1[i] != -1 && i<Kp;i++){}
			pare_count = i;
		...
		if(pare_count < Np){
			...
			for(i=0;i<pare_count;i++){
				SubPare_n[i] = MainPare.Neigh_List1[i];
			...
			for(i=0;i<Np;i++){
				while_flag = 1;
				while(while_flag){
					tmp_rand = genrand_int32() % Kp;
					/*リストが空でなければ追加*/
					if(MainPare.Neigh_List1[tmp_rand] != -1){
						SubPare_n[i] = MainPare.Neigh_List1[tmp_rand];
						SubPare[i] = pop[SubPare_n[i]];
						SubPare[i].win = 0;
						/*個体と取り出して空にする*/
						MainPare.Neigh_List1[tmp_rand] = -1;
						while_flag = 0;
					}
				}
			}
  ...
  */
  ...
  printf("n:%d battle_all:%d battle_ave:%d time:%f\n",eval_n,battle_n,battle_n/END_STEP,time_count);
	SDL_Quit();
	return 0;
}
/****************
...
void AnsList1()
{
	...
	for(i=0;i<Ns;i++){
		for(j=0;j<Kp;j++){
			pop[i].Neigh_List1[j] = pop[i].obj[j+1];
		}
	}
}
/********************
...
void AnsList2()
{
	...
	for(i=0;i<Ns;i++){
		...
		for(j=1;j<Ko;j++){
			save_obj = pop[i].obj[j];
			/*近傍Ｋ番目までの個体の近傍にiが存在すればリストに加える*/
			for(k=0;k<Ko;k++){
				if(i == pop[save_obj].obj[k]){
					pop[i].Neigh_List2[obj_count] = pop[i].obj[j];
					obj_count++;
				}
			}
		}
...
*****************/
int Set_Nitch(int i){
	int tmp; /*近傍リスト内の個体の番号を一時的に保存しておく*/
	int j;
  if(pop[i].nitch == -1){
		pop[i].nitch = count_nitch;
  	/*近傍リストを見ていく*/
	  for(j=0;j<Ko && pop[i].Neigh_List2[j] != -1;j++){
	  	tmp = pop[i].Neigh_List2[j];
	  	Set_Nitch(tmp);
	  }
	  return 1;
	}
